...
static int tls1_P_hash(const EVP_MD *md,
                       ...
                       unsigned char *out, int olen)
{
    ...
    do {
        ...
        if (seed1) HMAC_Update(ctx, seed1, seed1_len);
        if (seed2) HMAC_Update(ctx, seed2, seed2_len);
        if (seed3) HMAC_Update(ctx, seed3, seed3_len);

        /* First generate A1 from the seed */
        if (!i)
            HMAC_Final(ctx, A1, &chunk);
        else if (i * chunk <= olen) {
            HMAC_Final(ctx, out + ((i-1) * chunk), NULL);
            /* calculate A(n+1) value */
            HMAC(md, sec, sec_len, A1, chunk, A1, NULL);
        } else {
            HMAC_Final(ctx, A1, NULL);
            memcpy(out + ((i-1) * chunk), A1, olen % chunk);
            break;
        }
        HMAC_Init_ex(ctx, NULL, 0, NULL, NULL);
        i++;
    } while ((i-1) * chunk <= olen);
                  ...
                  unsigned char *out, int olen)
{
    ...
    if (!tls1_P_hash(EVP_md5(), master_secret, len,
                     ...
                     seed3_len, out, olen) ||
        !tls1_P_hash(EVP_sha1(), master_secret + len, len,
                     seed1, seed1_len, seed2, seed2_len, seed3,
                     seed3_len, out2, olen))
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    handshake_md5 = EVP_MD_CTX_new();
    handshake_sha1 = EVP_MD_CTX_new();
 ...
 end_ctx:
    SSL_CTX_free(ctx);
 end_md:
    EVP_MD_CTX_free(handshake_md5);
    EVP_MD_CTX_free(handshake_sha1);
 end:
